// ---------------------------------------------------------------------------
// ascon_kat_sequence -- directed stimulus from the NIST reference vectors.
//
// Reads the stimulus half of the files produced by gen_kat_hex.py and drives
// them in order. It deliberately does NOT read kat_ct/kat_tag -- the expected
// results belong to the scoreboard, so the stimulus side cannot influence the
// check.
//
// Run xsim from ascon_hw_verified/ so the $readmemh paths resolve.
// ---------------------------------------------------------------------------
class ascon_kat_sequence extends uvm_sequence #(ascon_sequence_item);
  `uvm_object_utils(ascon_kat_sequence)
  ...
  function new(string name = "ascon_kat_sequence");
    super.new(name);
  endfunction
  ...
  task body();
    ...
    for (int v = 0; v < num_vectors; v++) begin
      ...
      req.key     = KAT_KEY;
      req.nonce   = KAT_NONCE;
      req.decr_en = 1'b0;                 // KAT vectors are encryption
      unpack(kat_pt, v*MAXBLK, nbytes_data, req.pt);
