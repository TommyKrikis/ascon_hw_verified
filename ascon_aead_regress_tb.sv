...
module ascon_aead_regress_tb;
    ...
    task automatic run_case(input string name);
        ...
        begin
            cases++;
            ct_n = 0;                       // reset the monitor for this case
            ...
            if (ct_n !== pt_blocks) begin
                errors++;
                $display("FAIL [%s] block count: got %0d exp %0d", name, ct_n, pt_blocks);
            end

            // --- full blocks: compare all 128 bits ---
            for (int i = 0; i < pt_blocks - 1; i++) begin
                if (ct_cap[i] !== exp_ct[i]) begin
                    errors++;
                    $display("FAIL [%s] ct[%0d]: got %032h exp %032h",
                             name, i, ct_cap[i], exp_ct[i]);
                end
            end

            // --- last block is partial: only the low pt_len bytes count.
            //     pt_len == 0 -> mask is 0 -> nothing to compare (correct:
            //     a pure-padding block contributes no ciphertext).
            mask = (pt_len == 0) ? 128'h0 : ({128{1'b1}} >> (128 - pt_len*8));
            if ((ct_cap[pt_blocks-1] & mask) !== (exp_ct[pt_blocks-1] & mask)) begin
                errors++;
                $display("FAIL [%s] ct[%0d] (last, masked): got %032h exp %032h",
                         name, pt_blocks-1,
                         ct_cap[pt_blocks-1] & mask, exp_ct[pt_blocks-1] & mask);
            end
            ...
            if (tag !== exp_tag) begin
                ...
                $display("PASS [%s]  (AD blk=%0d, PT blk=%0d, pt_len=%0d)",
                         name, ad_blocks, pt_blocks, pt_len);
            end
    ...
    initial begin
        ...
        ad_blocks = 8'd0; pt_blocks = 8'd1; pt_len = 4'd0;
        ...
        ad_blocks = 8'd0; pt_blocks = 8'd1; pt_len = 4'd4;
        ...
        ad_blocks = 8'd0; pt_blocks = 8'd2; pt_len = 4'd0;
        ...
        ad_blocks = 8'd0; pt_blocks = 8'd2; pt_len = 4'd4;
        ...
        ad_blocks = 8'd0; pt_blocks = 8'd3; pt_len = 4'd8;
        ...
        ad_blocks = 8'd1; pt_blocks = 8'd2; pt_len = 4'd4;
        ...
        ad_blocks = 8'd2; pt_blocks = 8'd2; pt_len = 4'd4;
        ...
        ad_blocks = 8'd3; pt_blocks = 8'd3; pt_len = 4'd1;
